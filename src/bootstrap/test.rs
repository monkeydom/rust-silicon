...
impl Step for Clippy {
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        let target_libs =
            builder.stage_out(compiler, Mode::ToolRustc).join(&self.host.triple).join(builder.cargo_dir());
...
impl Step for Distcheck {
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        builder.run(
            Command::new(build_helper::make(&builder.config.build.triple)).arg("check").current_dir(&dir),
        );
